    ...
    open override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (indexPath as NSIndexPath).section == 0 {
            ...
            if rule.bymonthday.count == 0 {
				if let monthday = occurrenceDateComponents.day {
					rule.bymonthday = [monthday]
				}
            }
            if rule.bymonth.count == 0 {
				if let month = occurrenceDateComponents.month {
					rule.bymonth = [month]
				}
            }
...
extension RecurrencePicker {
    ...
    fileprivate func updateRecurrenceRule(withSelectedIndexPath indexPath: IndexPath) {
        ...
        case 2:
			...
			if let weekDayRaw = occurrenceDateComponents.weekday, let weekday = EKWeekday(rawValue: weekDayRaw) {
    ...
    fileprivate func recurrenceRuleText() -> String? {
        return (selectedIndexPath as IndexPath).section == 1 ? recurrenceRule?.toText(on: occurrenceDate) : nil
    }
